...
function XPost({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-3.5 py-2.5">
        <div className="size-10 shrink-0 rounded-full bg-fill-subtle flex items-center justify-center overflow-hidden">
          {avatar ? (
            ...
            <span className="text-[14px] font-[590] text-foreground">{username[0]?.toUpperCase()}</span>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <span className="text-label-primary-bold text-[14px] truncate">{username}</span>
            {verified && (
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-label="Verified">
                <circle cx="7" cy="7" r="6" fill="var(--color-brand)" />
                <path d="M4 7L6 9L10 5" stroke="var(--color-card)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            )}
          </div>
          <span className="text-[12px] text-muted-foreground">@{handle}</span>
        </div>
        {/* X/Twitter logo */}
      ...
      {/* Image */}
      {image && (
        <img src={image} alt="" className="w-full rounded-[14px] mx-3.5 w-[calc(100%-28px)]" />
      )}

      {/* Timestamp */}
      {timestamp && (
        <div className="px-3.5 pt-1.5 pb-1">
          <time className="text-[12px] text-muted-foreground">{timestamp}</time>
        </div>
      )}
